...
void writeFileCondensed(string, CustomLinkedList<string> *);
...
int main(){
    ...
    if (filteredEntries.size() != 0) {
        ...
        writeFileCondensed("bitacora_sorted_" + firstMonth + "_" + to_string(firstDay) + "_" + lastMonth + "_" + to_string(lastDay) + "Condensed.txt", doubleLinkedList);
...
void writeFileCondensed(string fileName, CustomLinkedList<string>* list){
    fstream file;
    file.open(fileName, fstream::in | fstream::out | fstream::app);
    Node<string>* current = list->head;
    while(current != NULL){
        vector<string> entries;
        entries = sort(current->ipsNRep);
        for (int i = 0; i < entries.size(); i++) {
            file << entries[i] << endl;
        }
